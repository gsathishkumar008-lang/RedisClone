...
void initializeCommands() {
    commands = {
        ...
        {"LRANGE", [](Database& db, const Tokens& t) { if (t.size() != 4) return error("wrong number of arguments for 'lrange' command"); long long start = 0; long long stop = 0; if (!parseNumber(t[2], start) || !parseNumber(t[3], stop)) return error("value is not an integer or out of range"); std::vector<std::string> values; const int result = db.lrange(t[1], start, stop, values); if (result < 0) return error("WRONGTYPE operation against a key holding the wrong kind of value"); return array(values); }},
